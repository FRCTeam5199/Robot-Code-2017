...
public class NewPixy {
	...
	public int getStartOfDataChris2017_01_31() {
		...
		//  The expectation is that the block data will be read in the next routine for processing.
		...
		if ((word[0] == (byte) 0x55) && (word[1] == (byte) 0xAA))
		{
			...
			if ((word[0] == (byte) 0x56) && (word[1] == (byte) 0xAA))
		...
		if ((word[0] == (byte) 0x00) && (word[1] == (byte) 0x55))
		{
		...
		if ((word[0] == (byte) 0xAA) && (word[1] == (byte) 0x55))
		{
			// Check to see if this could be an offset sync word by looking at the next byte.
			tmp = getVariableSizeBuffer(1);
			if (tmp[0]==(byte)0xAA )
		...
		if ((word[0] == (byte) 0xAA) && (word[1] == (byte) 0x56))
		{
			// Check to see if this could be an offset sync word by looking at the next byte.
			tmp = getVariableSizeBuffer(1);
			if (tmp[0]==(byte)0xAA )
